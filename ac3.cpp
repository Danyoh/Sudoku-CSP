...
public:
    ...
    bool revise(sudoku &s, string x1, string x2) {
        /*
        REVISE
        This function will revise the domain of x1 based on the constraints of x2.
        It will return true if the domain of x1 was changed and false if it was not.
        */
        bool revised = false;
        vector<int> temp = s.domain[x1]; // temp vector to store the domain of x1
        for (int i = 0; i < temp.size(); i++) {
            bool consistent = false;
            for (int j = 0; j < s.domain[x2].size(); j++) {
                if (temp[i] != s.domain[x2][j]) // if the value in the domain of x1 is not in the domain of x2
                    consistent = true;          // then the value is consistent
            }
            if (!consistent) // if the value is not consistent
            {
                s.domain[x1].erase(s.domain[x1].begin() + i); // remove the value from the domain of x1
                revised = true;
            }
        }
        return revised;
